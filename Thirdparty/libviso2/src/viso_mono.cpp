...
namespace viso2 {
    ...
    vector<double> VisualOdometryMono::estimateMotion(vector<Matcher::p_match> p_matched) {
      ...
      x_plane.setMat(X_plane.getMat(1, 0, 2, -1), 0, 0);//ȡy-z������

      Matrix n(2, 1);
      //��������ϵĵ����������ͶӰ
      n.val[0][0] = cos(-param.pitch);    //y = 1
      n.val[1][0] = sin(-param.pitch);    //z = 0
      Matrix d = ~n * x_plane;
      ...
      for (int32_t i = 0; i < x_plane.n; i++) {
        if (d.val[0][i] > median / param.motion_threshold) {
          ...
          for (int32_t j = 0; j < x_plane.n; j++) {
            ...
            sum += exp(-dist * dist * weight);//֮��ľ���ԽС�������ĺ�Խ��������������ĵ���һ��ƽ���ϵĸ���Խ��
          }
          if (sum > best_sum) {
            best_sum = sum;
            best_idx = i;
    ...
    Matrix VisualOdometryMono::smallerThanMedian(Matrix &X, double &median) {
      ...
      vector<int32_t> idx;
      for (int32_t i = 0; i < X.n; i++) {
        dist.push_back(fabs(X.val[0][i]) + fabs(X.val[1][i]) + fabs(X.val[2][i]));//��������ľ���ֵ�͵���λ��
        idx.push_back(i);
      }
    ...
    void VisualOdometryMono::EtoRt(Matrix &E, Matrix &K, vector<Matcher::p_match> &p_matched, Matrix &X, Matrix &R,
                                   Matrix &t) {
