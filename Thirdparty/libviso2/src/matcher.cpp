    ...
    void Matcher::matchFeatures(int32_t method, Matrix *Tr_delta) {
      ...
      if (method == 0) {
        if (m1p2 == 0 || n1p2 == 0 || m1c2 == 0 || n1c2 == 0)
          return;
        if (param.multi_stage) if (m1p1 == 0 || n1p1 == 0 || m1c1 == 0 || n1c1 == 0)
          return;
        ...
        if (m1c2 == 0 || n1c2 == 0 || m2c2 == 0 || n2c2 == 0)
          return;
        if (param.multi_stage) if (m1c1 == 0 || n1c1 == 0 || m2c1 == 0 || n2c1 == 0)
          return;
        ...
        if (m1p2 == 0 || n1p2 == 0 || m2p2 == 0 || n2p2 == 0 || m1c2 == 0 || n1c2 == 0 || m2c2 == 0 || n2c2 == 0)
          return;
        if (param.multi_stage) if (m1p1 == 0 || n1p1 == 0 || m2p1 == 0 || n2p1 == 0 || m1c1 == 0 || n1c1 == 0 ||
                                   m2c1 == 0 || n2c1 == 0)
          return;
      }
...
///////////////////////
    ...
    void
    Matcher::nonMaximumSuppression(int16_t *I_f1, int16_t *I_f2, const int32_t *dims, vector<Matcher::maximum> &maxima,
                                   int32_t nms_n) {
    ...
    inline void
    Matcher::computeDescriptor(const uint8_t *I_du, const uint8_t *I_dv, const int32_t &bpl, const int32_t &u,
    ...
    inline void
    Matcher::computeSmallDescriptor(const uint8_t *I_du, const uint8_t *I_dv, const int32_t &bpl, const int32_t &u,
                                    const int32_t &v, uint8_t *desc_addr) {
    ...
    void
    Matcher::filterImageAll(uint8_t *I, uint8_t *I_du, uint8_t *I_dv, int16_t *I_f1, int16_t *I_f2, const int *dims) {
    ...
    void Matcher::computeFeatures(uint8_t *I, const int32_t *dims, int32_t *&max1, int32_t &num1, int32_t *&max2,
                                  int32_t &num2, uint8_t *&I_du, uint8_t *&I_dv, uint8_t *&I_du_full,
                                  uint8_t *&I_dv_full) {
      ...
      if (!param.half_resolution) {
        ...
        uint8_t *I_matching = createHalfResolutionImage(I, dims);//���������ͼ������ƥ��
        getHalfResolutionDimensions(dims, dims_matching);
        I_du = (uint8_t *) _mm_malloc(dims_matching[2] * dims_matching[1] * sizeof(uint8_t *), 16);
        I_dv = (uint8_t *) _mm_malloc(dims_matching[2] * dims_matching[1] * sizeof(uint8_t *), 16);//������ͼ����ݶ�ͼ��
        I_f1 = (int16_t *) _mm_malloc(dims_matching[2] * dims_matching[1] * sizeof(int16_t), 16);//������ͼ��blob�˲�ͼ��
        I_f2 = (int16_t *) _mm_malloc(dims_matching[2] * dims_matching[1] * sizeof(int16_t),
                                      16);//������ͼ���checkboard�˲�ͼ��
        I_du_full = (uint8_t *) _mm_malloc(dims[2] * dims[1] * sizeof(uint8_t *), 16);//ԭͼ����ݶ�ͼ��
        I_dv_full = (uint8_t *) _mm_malloc(dims[2] * dims[1] * sizeof(uint8_t *), 16);
        filter::sobel5x5(I_matching, I_du, I_dv, dims_matching[2], dims_matching[1]);//������ͼ����ݶȼ���
        filter::sobel5x5(I, I_du_full, I_dv_full, dims[2], dims[1]);
        filter::blob5x5(I_matching, I_f1, dims_matching[2], dims_matching[1]);//�����˲�ģ��
      ...
      if (num1 != 0) {
        max1 = (int32_t *) _mm_malloc(sizeof(Matcher::maximum) * num1,
                                      16);//����ֵ�����ڴ棬����ֵΪ�������λ�á�����ǿ�ȡ���������������
        int32_t k = 0;
    ...
    void Matcher::createIndexVector(int32_t *m, int32_t n, vector<int32_t> *k, const int32_t &u_bin_num,
    ...
    inline void
    Matcher::findMatch(int32_t *m1, const int32_t &i1, int32_t *m2, const int32_t &step_size, vector<int32_t> *k2,
      ...
      if (method == 0) {
        ...
        for (i1p = 0; i1p < n1p; i1p++) {
          ...
          if (Tr_delta) {
            ...
            findMatch(m2p, i2p, m2c, step_size, k2c, u_bin_num, v_bin_num, stat_bin, i2c, 1, true, use_prior, u2c_,
                      v2c_);
          } else {
          ...
          if (Tr_delta)
            findMatch(m1c, i1c, m1p, step_size, k1p, u_bin_num, v_bin_num, stat_bin, i1p2, 3, true, use_prior, u1p,
                      v1p);
          else
    ...
    */
    ...
    float Matcher::mean(const uint8_t *I, const int32_t &bpl, const int32_t &u_min, const int32_t &u_max,
