...
namespace viso2 {
    ...
    void
    Reconstruction::update(vector<Matcher::p_match> p_matched, Matrix Tr, int32_t point_type, int32_t min_track_length,
      ...
      for (vector<track>::iterator t = tracks_copy.begin(); t != tracks_copy.end(); t++) {
        ...
        if (t->last_frame == current_frame) {
          ...
          if (t->pixels.size() >= min_track_length) {
            ...
            if (initPoint(*t, p)) {
              if (pointType(*t, p) >= point_type) if (refinePoint(*t, p)) if (pointDistance(*t, p) < max_dist &&
    ...
    Reconstruction::updatePoint(const track &t, point3d &p, const FLOAT &step_size, const FLOAT &eps) {
    ...
    bool Reconstruction::computePredictionsAndJacobian(const vector<Matrix>::iterator &P_begin,
